    ...
    <div class="flex flex-col items-center justify-between grow xl:flex-row xl:px-6">
        <div
            ...
            <div class="hidden xl:block" 
                x-data="{ 
                    searchQuery: '',
                    results: { books: [], members: [] },
                    isOpen: false,
                    isLoading: false,
                    
                    search() {
                        if (this.searchQuery.length < 2) {
                            this.results = { books: [], members: [] };
                            this.isOpen = false;
                            return;
                        }
                        ...
                        fetch(`/api/search/global?q=${this.searchQuery}`)
                            .then(res => res.json())
                            .then(data => {
                                this.results = data;
                ...
                <div class="relative">
                    ...
                    <!-- Results Dropdown -->
                    <div x-show="isOpen" 
                        class="absolute left-0 mt-2 w-full max-h-96 overflow-y-auto rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-800 dark:bg-gray-900 z-99999"
                        style="display: none;">
                        ...
                        <div x-show="!isLoading">
                            <!-- Books Section -->
                            <template x-if="results.books.length > 0">
                                <div>
                                    <h3 class="px-4 py-2 text-xs font-semibold uppercase tracking-wider text-gray-500 bg-gray-50 dark:bg-gray-800/50">Buku</h3>
                                    <template x-for="book in results.books" :key="book.id">
                                        <a :href="book.url" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                                            <span x-text="book.icon" class="text-lg"></span>
                                            <div>
                                                <p x-text="book.title" class="text-sm font-medium text-gray-800 dark:text-white"></p>
                                                <p x-text="book.subtitle" class="text-xs text-gray-500"></p>
                                            </div>
                                        </a>
                                    </template>
                                </div>
                            ...
                            <!-- Members Section -->
                            <template x-if="results.members.length > 0">
                                <div>
                                    <h3 class="px-4 py-2 text-xs font-semibold uppercase tracking-wider text-gray-500 bg-gray-50 dark:bg-gray-800/50">Anggota</h3>
                                    <template x-for="member in results.members" :key="member.id">
                                        <a :href="member.url" class="flex items-center gap-3 px-4 py-3 hover:bg-gray-100 dark:hover:bg-gray-800 transition-colors">
                                            <span x-text="member.icon" class="text-lg"></span>
                                            <div>
                                                <p x-text="member.title" class="text-sm font-medium text-gray-800 dark:text-white"></p>
                                                <p x-text="member.subtitle" class="text-xs text-gray-500"></p>
                                            </div>
                                        </a>
                                    </template>
                                </div>
                            ...
                            <!-- No Results -->
                            <div x-show="results.books.length === 0 && results.members.length === 0" class="p-4 text-center text-sm text-gray-500">
                                Tidak ada hasil ditemukan untuk "<span x-text="searchQuery"></span>"
                            </div>
                        </div>
                    </div>
                </div>
