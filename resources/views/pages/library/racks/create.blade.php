...
@section('content')
    <div class="mb-6 flex items-center justify-between">
        <div>
            <h2 class="text-2xl font-bold text-gray-800 dark:text-white">Tambah Rak</h2>
            <p class="text-gray-500 dark:text-gray-400">Tambah lokasi penyimpanan baru</p>
        </div>
    ...
    <div class="max-w-3xl">
        <div class="rounded-xl border border-gray-200 bg-white shadow-sm dark:border-gray-800 dark:bg-gray-900">
            <div class="border-b border-gray-200 px-6 py-4 dark:border-gray-800">
                <h3 class="font-semibold text-gray-800 dark:text-white">Formulir Rak Baru</h3>
            </div>
            
            <form action="{{ route('racks.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div class="grid gap-6 sm:grid-cols-2">
                        ...
                        <div x-data="{ isNew: {{ old('name_type', 'existing') === 'new' ? 'true' : 'false' }} }">
                            <div class="flex flex-col gap-2">
                                ...
                                <div x-show="isNew" class="relative">
                                    <input type="text" name="name" x-ref="nameInput" value="{{ old('name') }}"
                                        class="w-full rounded-lg border border-gray-300 px-4 py-2.5 dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:border-blue-500 focus:ring-blue-500"
                    ...
                    <div>
                        ...
                        <div x-data="{ isNew: {{ old('location_type', 'existing') === 'new' ? 'true' : 'false' }} }">
                            <div class="flex flex-col gap-2">
                                ...
                                <div x-show="isNew" class="relative">
                                    <input type="text" name="location" x-ref="locInput" value="{{ old('location') }}"
                                        class="w-full rounded-lg border border-gray-300 px-4 py-2.5 dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:border-blue-500 focus:ring-blue-500"
                                ...
                                {{-- Hidden input to store chosen location when using select --}}
                                <input type="hidden" name="location" x-model="$refs.locInput.value" x-show="!isNew">
                            </div>
                        </div>
                        @error('location')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    ...
                    <div>
                        ...
                        <input type="number" name="capacity" id="capacity" value="{{ old('capacity', 50) }}" required min="1" max="500"
                            class="w-full rounded-lg border border-gray-300 px-4 py-2.5 dark:border-gray-700 dark:bg-gray-800 dark:text-white focus:border-blue-500 focus:ring-blue-500">
                        @error('capacity')
                            <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                        @enderror
